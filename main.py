...
def admin_page():
    ...
    if request.method == 'POST':
        ...
        change_req = put(f'https://localhost:5000/api/users/{form.user_id.data}', json={'role_id': form.role_id.data},
...
def admin_user_page(user_id):
    ...
    if request.method == 'POST':
        if 'save_submit' in request.form:
            ...
            post_status = put(f'https://localhost:5000/api/users/{user_id}', json=user_json)
        ...
        if 'delete_submit' in request.form:
            if current_user.role_id != 4:
                return render_template('fail.html', message='У вас нет прав на эти действия')
            orders = get(f'https://localhost:5000/api/orders?id_user={user_id}').json()['orders']
            if len(orders) > 0:
            ...
            del_user = delete(f'https://localhost:5000/api/users/{user_id}', headers={'Authorization': 'Bearer ' + tokn})
            del_notif = delete(f'https://localhost:5000/api/notification?id_user={user_id}')
            del_appeal = delete(f'https://localhost:5000/api/appeal?id_user={user_id}')
...
def admin_send_appeal_answer(appeal_id):
    ...
    appeal = get(f'https://localhost:5000/api/appeal/{appeal_id}')
    ...
    if request.method == 'POST':
        if 'save_submit' in request.form:
            ...
            answer_post = post(f'https://localhost:5000/api/notification', json=answer_notification)
        ...
        if 'delete_submit' in request.form:
            answer_post = delete(f'https://localhost:5000/api/appeal/{appeal_id}')
            if answer_post.status_code == 200:
...
def admin_product_create():
    ...
    if request.method == 'POST':
        ...
        resp = post(f'https://localhost:5000/api/full_product', data=form.to_dict(), files=files)
...
def admin_product_page(product_id):
    form = ProductForm()
    product_req = get(f'https://localhost:5000/api/products/{product_id}')
    ...
    if request.method == 'POST':
        if 'save_sub' in request.form:
            files = {
                ...
                for
                i in range(len(form.images.data))}
            resp = put(f'https://localhost:5000/' + 'api/full_product?description_id={}&product_id={}'.format(
        ...
        if 'delete_submit' in request.form:
            tokn = generate_token({'id': current_user.id})
            resp = delete(f'https://localhost:5000/api/products/{product_id}',
                          headers={'Authorization': 'Bearer ' + tokn})
            if resp.status_code == 200:
...
def admin_notification_create():
    ...
    if request.method == 'POST':
        ...
        if notif_json['public']:
            resp = post(f'https://localhost:5000/api/notification?public=true', json=notif_json)
        else:
            resp = post(f'https://localhost:5000/api/notification', json=notif_json)
        if resp.status_code == 200:
...
def home_page():
    ...
    if form.validate_on_submit():
        ...
        post(f'https://localhost:5000/api/appeal', json=appeal_data).json()
        post(f'https://localhost:5000/api/notification', json=notif_data).json()
...
@app.route('/catalog')
def catalog():
    products = get(f'https://localhost:5000/api/products').json()
    return render_template('catalog.html', products=products)


@app.route('/catalog/<int:product_id>', methods=['GET', 'POST'])
def product(product_id):
    prod = get(f'https://localhost:5000/api/products/{product_id}').json()['products']
    products = get(f'https://localhost:5000/api/products').json()
    if request.method == 'POST':
        if not current_user.is_authenticated:
            return redirect('/login')
        ...
        response_order = post(f'https://localhost:5000/api/orders', json=json_order).json()
        ...
        notification = {'title': 'Регистрация заказа №' + str(response_order['id']),
                        ...
                        'id_user': current_user.id}
        post(f'https://localhost:5000/api/notification', json=notification)
...
def register():
    ...
    if request.method == 'POST':
        if form.validate():
            try_post = post(f'https://localhost:5000/api/users',
            ...
            post_notif = post(f'https://localhost:5000/api/notification', json=notif_json)
...
def login():
    form = UserForm()
    if request.method == 'POST':
        login_post = post(f'https://localhost:5000/api/login',
                          json={'email': form.email.data, 'password': form.password.data})
...
def profile(user_id):
    ...
    orders = get(f'https://localhost:5000/api/orders?id_user={user_id}')
    order = None
...
def user_info(user_id):
    ...
    if request.method == 'POST':
        ...
        if 'delete_submit' in request.form:
            ...
            del_user = delete(f'https://localhost:5000/api/users/{user_id}', headers={'Authorization': 'Bearer ' + tokn})
            del_notif = delete(f'https://localhost:5000/api/notification?id_user={user_id}')
            del_user = delete(f'https://localhost:5000/api/appeal?id_user={user_id}')
            logout_user()
            return redirect('/')
...
def order_page(order_id):
    form = EditOrderForm()
    order_req = get(f'https://localhost:5000/api/orders/{order_id}')
    ...
    if request.method == 'POST':
        if 'save_submit' in request.form and check_if_admin(current_user):
            ...
            resp = put(f'https://localhost:5000/api/orders/{order_id}', json=data_order)
            if resp.status_code == 200:
                return redirect(f'/order/{order_id}')

        if 'delete_submit' in request.form and current_user.role_id == 4:
            tokn = generate_token({'id': current_user.id})
            req = delete(f'https://localhost:5000/api/orders/{order_id}',
                         headers={'Authorization': 'Bearer ' + tokn})
...
def user_notifications(user_id):
    ...
    if request.method == 'POST':
        if 'delete_submit' in request.form:
            ...
            res = delete(f'https://localhost:5000/api/notification/{id}')

        if 'read_submit' in request.form:
            id = request.form.get('read_submit')
            res = put(f'https://localhost:5000/api/notification/{id}', json={'read': True})
        return redirect(f'/profile/{user_id}/notifications')

    notifications = get(f'https://localhost:5000/api/notification?id_user={user_id}').json()
